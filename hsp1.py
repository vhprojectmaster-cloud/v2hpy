...
# ============================================================
# ACADEMIC SUNNY WEEKDAY HEMS + PV + V2H FUZZY RELAY CONTROL
#
# Relay IN2 -> Raspberry Pi GPIO27 / physical pin 13
#
# Relay/Lamp meaning:
# Relay ON  = V2H discharging active
# Relay OFF = hold / PV charging / off-peak charging
#
# Scenario:
# - Sunny weekday
# - 4 kWp rooftop PV
# - 60 kWh EV battery
# - 3.3 kW V2H discharge limit
# - EV available all day
# - Noon limited V2H discharge
# - Afternoon PV charging
# - Evening V2H peak shaving
# - Night off-peak charging to morning reserve
#
...
# If the lamp becomes opposite again, change True to False.
...
SOC_RESERVE_NORMAL = 35.0
MORNING_RESERVE_SOC = 55.0
...
def required_reserve_soc(hour):
    if hour >= 22 or hour <= 8:
        return MORNING_RESERVE_SOC

    return SOC_RESERVE_NORMAL
...
def fuzzy_v2h_controller(hour, load, pv, soc, available, price, carbon):
    net_load = load - pv
    solar_ratio = pv / PV_REFERENCE_KW

    reserve_soc = required_reserve_soc(hour)
    soc_margin = soc - reserve_soc

    if available == 0:
        return "EV_NOT_AVAILABLE", 0.0, False, 0.0, net_load, solar_ratio, soc_margin

    if soc <= SOC_MIN:
        return "SOC_MIN_PROTECTION", 0.0, False, 0.0, net_load, solar_ratio, soc_margin

    # --------------------------------------------------------
    # PV charging during afternoon surplus
    ...
    # --------------------------------------------------------
    # Night off-peak charging to morning reserve
    # Relay OFF because it is charging, not V2H discharge.
    # --------------------------------------------------------
    ...
    margin_unsafe = trapezoid(soc_margin, -50.0, -50.0, 0.0, 5.0)
    ...
    rules.append((margin_unsafe, 0.0))
    ...
    if fuzzy_score >= 55.0 and net_load > 0 and soc_margin > 0:
        ...
        if is_noon_v2h_window(hour):
            ...
            decision = "NOON_LIMITED_V2H_DISCHARGE"

        elif is_evening_peak_window(hour):
            ev_power = min(EV_MAX_DISCHARGE_KW, net_load)

            if fuzzy_score >= 85.0:
                decision = "EVENING_STRONG_V2H_DISCHARGE"
            else:
                decision = "EVENING_MEDIUM_V2H_DISCHARGE"

        else:
            ev_power = min(1.0, net_load)
            decision = "GENERAL_V2H_DISCHARGE"
    ...
    return decision, ev_power, relay_on, fuzzy_score, net_load, solar_ratio, soc_margin
...
def main():
    ...
    log_file = "logs/academic_sunny_v2h_fuzzy_log.csv"
    ...
    print("===================================================")
    print(" ACADEMIC SUNNY WEEKDAY V2H FUZZY DEMO")
    print(" Relay ON  = V2H discharge active")
    print(" Relay OFF = hold / PV charging / off-peak charging")
    ...
    try:
        for i, hour in enumerate(hours):
            ...
            decision, ev_power, relay_on, fuzzy_score, net_load, solar_ratio, soc_margin = fuzzy_v2h_controller(
                hour,
                load,
                pv,
                ev_soc,
            ...
            results.append({
                ...
                "required_reserve_soc": required_reserve_soc(hour),
